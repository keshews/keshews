...
private:
  ...
  std::unordered_map<std::string, tokenType> keywords = {
      ...
      {"for", tokenType::FOR},       {"while", tokenType::WHILE},   {"int", tokenType::INT},
      {"double", tokenType::DOUBLE}, {"string", tokenType::STRING}, {"bool", tokenType::BOOL},
  ...
  void alpha_handler() {
    std::string str;
    while (!is_eof() && (current() == '_' || isdigit(current()) || isalpha(current()))) {
      str += advance();
    }
  ...
  void print_error(std::string msg) {
    std::cout << "Error: " << msg << " at line " << line << " and column " << pos - s_pos + 1 << std::endl;
    exit(1);
    return;
  }
